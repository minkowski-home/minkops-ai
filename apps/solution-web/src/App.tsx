...
function initialActivity(): ActivityItem[] {
  ...
  return [
    { id: "activity-escalation", agentName: "Kall", summary: "Found three SLA breaches for TechFlow Inc and held the account for your call.", time: "08:43", outcome: "waiting" },
    { id: "activity-lead", agentName: "Scout", summary: "Enriched Marcus Wren at NovaBuild and queued the record because confidence is below your threshold.", time: "08:21", outcome: "waiting" },
    ...messages.reverse()
  ];
}
...
function ProtectedConsole() {
  ...
  return <Routes>
    {solution.ui.navigation.map((item) => (
      <Route key={item.route} path={`/${item.route}`} element={<ConsoleShell title={item.label} productName={solution.ui.productName} navigation={solution.ui.navigation.map((navigationItem) => ({ ...navigationItem }))} pendingCount={pendingCount} collapsed={collapsed} onToggleSidebar={() => setCollapsed((value) => !value)}>{screen(item.route)}</ConsoleShell>} />
    ))}
