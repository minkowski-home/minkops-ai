...
This module contains the public-facing marketing site for pluseleven. The experience runs on a Vite + React + TypeScript frontend with a Node.js + TypeScript API for health checks and future marketing endpoints.
...
- `api/backend/` — Node.js + TypeScript API service (health check today, marketing endpoints later).
...
The corporate site ships with a minimal Node.js backend that serves health checks for orchestration tooling.

1. `cd apps/corporate-website/api/backend`
2. `npm install`
3. `npm run dev` to start the API on `http://127.0.0.1:5000`

The API entry point lives in `api/backend/src/index.ts`, so you can add new endpoints or integrate marketing automation without touching the React layer.
